...
class PinchImageView : ImageView {
    ...
    fun zoomMaskTo(mask: RectF?, duration: Long) {
        ...
        if (duration <= 0 || mMask == null) {
            ...
            //执行mask动画
            mMaskAnimator = MaskAnimator(mMask as RectF, mask, duration)
            mMaskAnimator!!.start()
        }
    }
    ...
    fun addOuterMatrixChangedListener(listener: OuterMatrixChangedListener?) {
        ...
        if (mDispatchOuterMatrixChangedLock == 0) {
            if (mOuterMatrixChangedListeners == null) {
                mOuterMatrixChangedListeners = ArrayList<OuterMatrixChangedListener>()
            }
            ...
            if (mOuterMatrixChangedListenersCopy == null) {
                if (mOuterMatrixChangedListeners != null) {
                    ...
                    mOuterMatrixChangedListenersCopy = ArrayList<OuterMatrixChangedListener>()
                }
            }
